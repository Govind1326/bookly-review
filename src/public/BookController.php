...
class BookController extends Controller
{
    ...
    {
        $books = Book::withAvg('reviews', 'rating')->where('deleted', '0')->orderBy('created_at', 'DESC')->get();
    ...
    public function store(Request $req)
    {
        ...
        if (!empty($req->image)) {
            $rules['image'] = 'image|mimes:jpeg,png,jpg,gif,svg|max:8192';
        }
    ...
    public function saveReview(Request $req)
    {
        ...
        if($countReview>0){
            session()->flash('error','You can only submit one review per book.');
            return response()->json(['success'=>'You can only submit one review per book.'],200);    
        }
        ...
        $bookReview->save();
        session()->flash('success','Review submitted successfully & pending for approval.');
        return response()->json(['success'=>'Review submitted successfully & pending for approval.'],200);
    }
}
