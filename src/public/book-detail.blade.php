            ...
            <div class="col-md-12">
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-8">
                        ...
                        <button id="toggleButton" class="btn btn-secondary mt-3">Start Reading</button>
...
<script>
   $(document).ready(function () {
    ...
    // Function to highlight the chunk currently being spoken
    function highlightChunk(index) {
        // Remove highlight from all text
        $('#description p').removeClass('highlight');
        // Highlight the current chunk
        var chunk = chunks[index];
        var startIndex = $('#description p').text().indexOf(chunk);
        var endIndex = startIndex + chunk.length;
        var highlightedText = $('#description p').text().slice(0, startIndex) +
            `<span class="highlight">${chunk}</span>` +
            $('#description p').text().slice(endIndex);
        $('#description p').html(highlightedText);
    }
    ...
    $('#toggleButton').click(function () {
        if (isReading) {
            ...
            synth.cancel();
            $(this).text('Start Reading');  // Change button text to "Start Reading"
            isReading = false;
            $('#description p').removeClass('highlight');
        } else {
            ...
            function speakNextChunk() {
                if (chunkIndex < chunks.length) {
                    ...
                    $(this).text('Start Reading');  // Reset button text when finished
                    isReading = false;
                    $('#description p').removeClass('highlight');
                }
            }
            ...
            $(this).text('Stop Reading');  // Change button text to "Stop Reading"
            isReading = true;
            speakNextChunk();
        }
    });
